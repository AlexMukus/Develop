...
- **Автоматическое определение раскладки клавиатуры (v1.2.0)**:
  - каталог топ-50 популярных моделей по VID/PID (Logitech, Razer, Corsair, SteelSeries, Keychron, Ducky и др.) — раскладка применяется молча при подключении;
  - маркерная эвристика для неизвестных моделей: подсказка «нажмите Enter на цифровом блоке (если есть) и клавишу слева от левого Shift» с диалогом предложения;
  - привязка «устройство → раскладка» сохраняется в `settings.json` (ключ `VID_XXXX&PID_YYYY`, для ноутбучных клавиатур — путь устройства) и применяется автоматически при повторном подключении;
  - кнопка «Определить автоматически» рядом с выбором раскладки — ручной запуск визарда.
